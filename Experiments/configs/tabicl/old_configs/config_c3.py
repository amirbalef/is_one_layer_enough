def generate_config_list():
    ...
    def disabling_attn_layer_config(t, m):
        config = {}
        config["model"] = "tabicl"
        config["model_name"] = f"{config['model']}_c3_{t[0]}{m}"
        config["desc"] = f"{config['model']} with disabling attention layer {t} {m}"
        ...
        for layer in range(3):
            if m == layer and t == "col":
                col_layers_info.append(( layer ,  {"attn1":{"w_attn_on_query": 0.0, "w_attn_on_support": 0.0, "w_ffn_on_support":1.0, "w_ffn_on_query": 1.0}, 
                "attn2":{"w_attn_on_query": 0.0, "w_attn_on_support": 0.0, "w_ffn_on_support":1.0, "w_ffn_on_query": 1.0}} ))
            else:
        ...
        for layer in range(3):
            if m == layer and t == "row":
                row_layers_info.append(( layer ,  {"w_attn_on_query": 0.0, "w_attn_on_support": 0.0, "w_ffn_on_support":1.0, "w_ffn_on_query": 1.0} ))
            else:
        ...
        for layer in range(12):
            if m == layer and t == "predictor":
                predictor_layers_info.append(( layer ,  {"w_attn_on_query": 0.0, "w_attn_on_support": 0.0, "w_ffn_on_support":1.0, "w_ffn_on_query": 1.0} ))
            else:
    ...
    for t in types:
        ...
        for m in range(max_m):
            new_config = disabling_attn_layer_config(t, m)
