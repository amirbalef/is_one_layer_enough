...
from ticla.models.utils import load_model_config, load_model_class, load_model_inference, load_model_weights


warnings.filterwarnings("ignore", category=FutureWarning, module="sklearn")


class TICLAClassifier(ClassifierMixin, BaseEstimator):
    """Tabular In-Context Learning Adapter with scikit-learn interface.
    ...
    model_name : Optional[str | Path] = None
    ...
    def _load_model(self):
        if self.model_name == None:
            self.model_name  = "nanotabpfn"
        
        self.cfg = load_model_config(self.model_name)
        Model = load_model_class(self.cfg)

        self.model_ = Model(**self.cfg.model.__dict__)
        self.model_ = load_model_weights(self.model_, self.cfg, device=self.device_)
        self.model_.eval()

        self.inference_config_ = load_model_inference(
            self.cfg,
