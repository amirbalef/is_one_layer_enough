...
from ticla.priors.tabicl_v1.prior_config import DEFAULT_FIXED_HP, DEFAULT_SAMPLED_HP

warnings.filterwarnings(
    "ignore", message=".*The PyTorch API of nested tensors is in prototype stage.*", category=UserWarning
)

from ticla.models.utils import load_model_config
...
class SavePriorDataset:
    ...
    def __init__(self, config):
        self.config = config
        self.save_dir = Path(self.config.prior.save_dir)
        ...
        self.prior = PriorDataset(
            ...
            num_threads_per_generate=self.config.prior.num_threads_per_generate,
    ...
    def save_batch_sparse(self, batch_idx, X, y, d, seq_lens, train_sizes):
        ...
        if self.config.prior.seq_len_per_gp:
    ...
    def run(self):
        ...
        print(f"Generating {self.config.prior.num_batches} batches starting from index {self.config.prior.resume_from}")

        for batch_idx in tqdm(
            range(self.config.prior.resume_from, self.config.prior.resume_from + self.config.prior.num_batches),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_config", type=str, default="nanotabpfn", help="Directory to save the generated data")
