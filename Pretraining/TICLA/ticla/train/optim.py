...
def get_scheduler(config, optimizer):
    ...
    if config.training.warmup_proportion >= 0:
        warmup_steps = config.training.max_steps * config.training.warmup_proportion
    else:
    ...
    if config.training.scheduler == "constant":
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=config.training.max_steps
        )
    elif config.training.scheduler == "cosine_warmup":
        scheduler = get_cosine_schedule_with_warmup(
            optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=config.training.max_steps
        )
    elif config.training.scheduler == "cosine_with_restarts":
        scheduler = get_cosine_with_restarts(
            optimizer=optimizer,
            num_warmup_steps=warmup_steps,
            num_training_steps=config.training.max_steps,
            num_cycles=config.training.cosine_num_cycles,
            amplitude_decay=config.training.cosine_amplitude_decay,
            lr_end=config.training.cosine_lr_end,
        )
    elif config.training.scheduler == "polynomial_decay_warmup":
        scheduler = get_polynomial_decay_schedule_with_warmup(
            optimizer=optimizer,
            num_warmup_steps=warmup_steps,
            num_training_steps=config.training.max_steps,
            lr_end=config.training.poly_decay_lr_end,
            power=config.training.poly_decay_power,
        )
    else:
