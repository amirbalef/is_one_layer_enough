...
class TransformerModel(nn.Module):
    ...
    def embeddings_enc(self):
        ...
        embeddings = []
        for l in self.transformer_encoder.layers:
            embeddings.append(l.embeddings)
        return embeddings

    def get_preds_all_layers(self):
        return self.preds_all_layers
    ...
    def forward(self, src, src_mask=None, single_eval_pos=None, layers_info = None):
        ...
        r = output[single_eval_pos+len(style_src)+(self.global_att_embeddings.num_embeddings if self.global_att_embeddings else 0):]

        # self.preds_all_layers = []
        # for l in self.transformer_encoder.layers:  # noqa: E741
        #     if l.embeddings is None:
        #         self.preds_all_layers.append(None)
        #     else:
        #         l_o = self.decoder(l.embeddings)
        #         self.preds_all_layers.append(
        #             l_o[
        #                 single_eval_pos
        #                 + len(style_src)
        #                 + (
