...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, device='auto', base_path= base_path.parent / "weights" / base_path.name , model_string='',
                 N_ensemble_configurations=3, no_preprocess_mode=False, multiclass_decoder='permutation',
                 feature_shift_decoder=True, only_inference=True, random_state=0, no_grad=True, batch_size_inference=32,
                 subsample_features=False, layers_info = None,finetuned_decoders_path : str | None = None,):
        """
        ...
        self.ensemble_configurations = None
    ...
    def fit(self, X, y, overwrite_warning=True):
        """
    ...
    def get_all_contribution_scores(self):
        ...
        for l, info in self.layers_info:
            layer = self.model[2].transformer_encoder.layers[l]
    ...
    def get_all_layers_embeddings(self):
        ...
        for l, info in self.layers_info:
            layer = self.model[2].transformer_encoder.layers[l]
            embeddings.append(layer.embeddings)
        return embeddings
    ...
    def get_all_layers_predictions(self, embeddings = None, decoder_type =  "default"):
        ...
        for l, emb in enumerate(embeddings):
            device = next(self.model[2].parameters()).device
            if decoder_type ==  "default":
                decoder = self.get_decoder()
            if decoder_type ==  "finetuned":
                decoder = self.finetuned_decoders[l].to(device)
            outputs = decoder(emb.to(device))
            output= None
            for i, ensemble_configuration in enumerate(self.ensemble_configurations):
                ...
                output_ = output_[:, :, 0:len(self.classes_)] / torch.exp(self.temperature if self.temperature is not None else torch.log(torch.tensor([0.8]))).to(device)
            ...
            output = output / len(self.ensemble_configurations)
            output = torch.nn.functional.softmax(output, dim=-1)
            p = torch.transpose(output, 0, 1).squeeze(0).cpu().numpy()
    ...
    def get_feature_encoder_embeddings(self):
        return [self.model[2].transformer_encoder.layers[self.layers_info[0][0]].in_embeddings]
    ...
    def predict_proba(self, X, normalize_with_test=False, return_logits=False):
        ...
        prediction, ensemble_configurations = transformer_predict(
            self.model[2],
            X_full,
            y_full,
            eval_pos,
    ...
    def predict_with_proba(self, X):
        p = self.predict_proba(X, normalize_with_test=False)
        y = np.argmax(p, axis=-1)
        y = self.classes_.take(np.asarray(y, dtype=np.intp))
        return y, p
    ...
    def model_inference(self,  X, y , support_size, max_features = 100):
        self.model[2].to(X.device)
        y_full = torch.cat([y, torch.zeros( (X.shape[0], X.shape[1] - support_size) , device=y.device, dtype=y.dtype)],dim=1)
        X_full =torch.cat( [X,torch.zeros((X.shape[0], X.shape[1], max_features - X.shape[2])).to(X.device)], -1)
        
        src = (X_full, y_full)
        output = self.model[2](src, src_mask=int(support_size), single_eval_pos=int(support_size), layers_info = self.layers_info)
        return output
    ...
    def predict(eval_xs, eval_ys, used_style, softmax_temperature, return_logits):
        ...
        with inference_mode_call:
            ...
            output = model(
                    ...
                    single_eval_pos=eval_position, layers_info = layers_info)[:, :, 0:num_classes]
...
#                return 'robust_all'
    ...
    inputs, labels = [], []
    start = time.time()
    #print("ensemble_configurations", len(ensemble_configurations))
    ...
    for batch_input, batch_label in zip(inputs, labels):
        ...
        #print("batch_input, softmax_temperature_", batch_input.shape, softmax_temperature_)
        import warnings
    ...
    return output, ensemble_configurations
