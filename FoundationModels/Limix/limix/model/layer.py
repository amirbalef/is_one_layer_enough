...
class EncoderBaseLayer(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.layer_arch == 'fmfmsm':
            ...
            self.layer_steps = [
                                ("attn_features", partial(
                                    self.call_features_attention,
                                    index=0
                                )),
                                ("mlp", self.mlp[0]),
                                ("second_attn_features", partial(
                                    self.call_features_attention,
                                    index=1
                                )),
                                ("second_mlp",self.mlp[1]),
                                ("attn_sequence", partial(
                                    self.call_sequence_attention,
                                    index=0
                                )),
                                 ("third_mlp",self.mlp[2])
            ]
    ...
    def forward(self, x: torch.Tensor, feature_atten_mask: torch.Tensor, eval_pos: int, task_info = {},**kwargs) -> tuple[torch.Tensor,torch.Tensor | None,torch.Tensor | None]:
        ...
        sample_attention=None
        for idx, ( (sublayer_name, sublayer), layer_norm) in enumerate(zip(self.layer_steps, self.layer_norms)):
            residual = x
            if self.pre_norm:
                x = layer_norm(x)
            ...
            self.component_contribution(sublayer_name, x, residual, eval_pos, task_info)

            x[:, :eval_pos] = (
                x[:, :eval_pos] * task_info.get("w_"+ sublayer_name + "_on_support", 1.0)
            )
            x[:, eval_pos:] = (
                    x[:, eval_pos:] * task_info.get("w_"+ sublayer_name + "_on_query", 1.0)
                )
            x = x + residual
        ...
        if not self.pre_norm:
            in_embeddings  = in_embeddings.to(device=x.device)
            for layer_norm in self.layer_norms:
                in_embeddings = layer_norm(in_embeddings)
            in_embeddings = in_embeddings.detach().cpu()
        self.component_contribution("layer", (self.out_embeddings - in_embeddings), (in_embeddings), eval_pos, task_info)
...
class LayerStack(nn.Module):
    ...
    def forward(self, x, layers_info = None, **kwargs):
        if layers_info is None:
            for idx,layer in enumerate(self.layers):
                kwargs["layer_idx"] = idx
                x,feature_attention,sample_attention = layer(x,**kwargs)
        else:
            for idx, task_info in layers_info:
                layer = self.layers[idx]
                kwargs["layer_idx"] = idx
                x,feature_attention,sample_attention = layer(x, task_info = task_info,**kwargs)
