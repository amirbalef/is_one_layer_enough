...
class RowInteraction(nn.Module):
    ...
    def _aggregate_embeddings(self, embeddings: Tensor, key_mask: Optional[Tensor] = None, layers_info= None) -> Tensor:
        ...
        outputs = self.tf_row(embeddings, key_padding_mask=key_mask, layers_info= layers_info)  # (B, T, H+C, E)
    ...
    def _inference_forward(self, embeddings: Tensor, mgr_config: MgrConfig = None, layers_info = None) -> Tensor:
        ...
        representations = self.inference_mgr(
            self._aggregate_embeddings, inputs=OrderedDict([("embeddings", embeddings), ("layers_info", layers_info)])
        )

        return representations  # (B, T, C*E)

    def forward(self, embeddings: Tensor, d: Optional[Tensor] = None, mgr_config: MgrConfig = None, layers_info = None) -> Tensor:
        ...
        if self.training:
            ...
            representations = self._inference_forward(embeddings, mgr_config, layers_info = layers_info)
