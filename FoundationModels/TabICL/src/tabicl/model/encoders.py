...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        for l, info  in layers_info:
            block = self.blocks[l]
            out = block(q=out, key_padding_mask=key_padding_mask, attn_mask=attn_mask, rope=self.rope, task_info = info)
...
class SetTransformer(nn.Module):
    ...
    def forward(self, src: Tensor, train_size: Optional[int] = None, layers_info = None) -> Tensor:
        """Process input through the stacked blocks.
        ...
        for l, info  in layers_info:
            block = self.blocks[l]
            out = block(out, train_size, task_info = info)
        return out
