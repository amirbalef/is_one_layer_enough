...
class ColEmbedding(nn.Module):
    ...
    def _compute_embeddings(self, features: Tensor, train_size: Optional[int] = None, layers_info = None) -> Tensor:
        ...
        src = self.tf_col(src, train_size,  layers_info)
    ...
    def _inference_forward(
        ...
        layers_info: Optional[List[List[int]]] = None,
    ) -> Tensor:
        ...
        if feature_shuffles is None:
            ...
            embeddings = self.inference_mgr(
                self._compute_embeddings, inputs=OrderedDict([("features", features), ("train_size", train_size), ("layers_info", layers_info)])
            )  # (B, H+C, T, E)
        else:
            B = X.shape[0]
            ...
            first_embeddings = self.inference_mgr(
                ...
                inputs=OrderedDict([("features", features), ("train_size", train_size), ("layers_info", layers_info)]),
                output_repeat=B,
            )  # (H+C, T, E)
    ...
    def forward(
        ...
        if self.training:
            ...
            embeddings = self._inference_forward(X, train_size, feature_shuffles, mgr_config, layers_info)
