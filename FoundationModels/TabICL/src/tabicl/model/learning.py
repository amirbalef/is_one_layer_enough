...
class ICLearning(nn.Module):
    ...
    def __init__(
        ...
        self.num_classes = max_classes  # Will be set during training or inference
    ...
    def _icl_predictions(self, R: Tensor, y_train: Tensor, layers_info = None) -> Tensor:
        ...
        src = self.tf_icl(R, attn_mask=train_size, layers_info = layers_info) 
        if self.norm_first:
            src = self.ln(src)
    ...
    def _predict_standard(
        ...
        out = self.inference_mgr(
            self._icl_predictions, inputs=OrderedDict([("R", R), ("y_train", y_train), ("layers_info", layers_info)]), auto_batch=auto_batch
        )
    ...
    def _predict_hierarchical(self, R_test: Tensor, softmax_temperature: float = 0.9, layers_info = None) -> Tensor:
    ...
    def _inference_forward(
        ...
        if num_classes <= self.max_classes:
            ...
            out = self._predict_standard(
                R, y_train, return_logits=return_logits, softmax_temperature=softmax_temperature, layers_info = layers_info
            )
        else:
            ...
            for ri, yi in zip(R, y_train):
                ...
                probs = self._predict_hierarchical(ri[train_size:], layers_info = layers_info)
                out.append(probs)
    ...
    def forward(
        ...
        if self.training:
            train_size = y_train.shape[1]
            out = self._icl_predictions(R, y_train, layers_info = layers_info)
            out = out[:, train_size:]
        else:
            out = self._inference_forward(R, y_train, return_logits, softmax_temperature, mgr_config, layers_info = layers_info)
