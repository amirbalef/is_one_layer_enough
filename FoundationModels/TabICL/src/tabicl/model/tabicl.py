...
class TabICL(nn.Module):
    ...
    def _train_forward(
        ...
        representations = self.row_interactor(
            self.col_embedder(X, d=d, train_size=None if embed_with_test else train_size, layers_info=col_layers_info), d=d, layers_info=row_layers_info,
        )

        # Dataset-wise in-context learning
        out = self.icl_predictor(representations, y_train=y_train, layers_info=predictor_layers_info)
    ...
    def _inference_forward(
        ...
        representations = self.row_interactor(
            self.col_embedder(
                ...
                layers_info=col_layers_info,
            ),
            mgr_config=inference_config.ROW_CONFIG,
            layers_info=row_layers_info,
        )
        ...
        out = self.icl_predictor(
            ...
            layers_info=predictor_layers_info,
        )
    ...
    def forward(
        ...
        if self.training:
            out = self._train_forward(X, y_train, d=d, embed_with_test=embed_with_test, col_layers_info=col_layers_info,
                row_layers_info=row_layers_info,
                predictor_layers_info=predictor_layers_info)
        else:
            out = self._inference_forward(
                ...
                col_layers_info=col_layers_info,
                row_layers_info=row_layers_info,
                predictor_layers_info=predictor_layers_info,
            )
