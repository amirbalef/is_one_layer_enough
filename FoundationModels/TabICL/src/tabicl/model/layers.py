...
class MultiheadAttentionBlock(nn.TransformerEncoderLayer):
    ...
    def __init__(
        ...
        self.init_weights()
        self.in_embeddings = None  # Store input embeddings for potential analysis
        self.out_embeddings = None  # Store output embeddings for potential analysis
    ...
    def component_contribution(self, component_name, out, src, single_eval_position: int, task_info={} ):
        ...
        if compute_component_contribution:
            ...
            if out_np.ndim == 3:
                ...
                src_np = src_np.transpose(1,0,2)
            if task_info.get("order_needs_transpose", False):
                out_np = out_np.transpose(1,0,2)
                src_np = src_np.transpose(1,0,2)
    ...
    def forward(
        ...
        """
        self.in_embeddings = q.clone().detach().cpu()
        components = ["attn", "ffn"]
        ...
        if self.norm_first:
            ...
            self.component_contribution("attn", attn, x, attn_mask, task_info)
            attn[:,:attn_mask,:] = attn[:,:attn_mask,:] * task_info.get("w_attn_on_support", 1.0)
            attn[:,attn_mask:,:] = attn[:,attn_mask:,:] * task_info.get("w_attn_on_query", 1.0)
            x = x + attn
            ffn =  self._ff_block(self.norm2(x))
            self.component_contribution("ffn", ffn, x, attn_mask, task_info)
            ffn[:,:attn_mask,:]  = ffn[:,:attn_mask,:] * task_info.get("w_ffn_on_support", 1.0)
            ffn[:,attn_mask:,:]  = ffn[:,attn_mask:,:] * task_info.get("w_ffn_on_query", 1.0)
            x = x + ffn
        else:
        ...
        self.component_contribution("layer", self.out_embeddings - in_embeddings, in_embeddings, attn_mask, task_info)
        return x
...
class InducedSelfAttentionBlock(nn.Module):
    ...
    def component_contribution(self, component_name, out: torch.Tensor, src: torch.Tensor, single_eval_position: int, task_info={}):
        compute_component_contribution = task_info.get("compute_component_contribution", False)
        if compute_component_contribution:
            out_np = out.detach().cpu().numpy().squeeze()
            src_np = src.detach().cpu().numpy().squeeze()
    ...
    def induced_attention(self, src: Tensor, train_size: Optional[int] = None, task_info = None) -> Tensor:
        ...
        *batch_shape, _, d_model = src.shape
        ...
        if train_size is None:
            hidden = self.multihead_attn1(ind_vectors, src, src, task_info = task_info["attn1"])
            self.component_contribution_scores["induced_attn1"] = self.multihead_attn1.component_contribution_scores
        else:
            hidden = self.multihead_attn1(ind_vectors, src[..., :train_size, :], src[..., :train_size, :], task_info = task_info["attn1"])
            self.component_contribution_scores["induced_attn1"] = self.multihead_attn1.component_contribution_scores

        out = self.multihead_attn2(src, hidden, hidden, task_info= task_info["attn2"])
        self.component_contribution_scores["induced_attn2"] = self.multihead_attn2.component_contribution_scores

        return out

    def forward(self, src: Tensor, train_size: Optional[int] = None, task_info = None) -> Tensor:
        """Apply induced self-attention to input sequence.
        ...
        if skip_mask.any():
            if skip_mask.all():
                ...
                out[~skip_mask] = self.induced_attention(src[~skip_mask], train_size, task_info = task_info)
                out[skip_mask] = self.skip_value
        else:
            out = self.induced_attention(src, train_size, task_info = task_info)

        self.out_embeddings = out.clone().detach().cpu()
        self.component_contribution("induced_attention_layer", self.out_embeddings -  self.in_embeddings, self.in_embeddings, train_size, task_info)
        return out
