...
class TabICLClassifier(ClassifierMixin, BaseEstimator):
    ...
    def __init__(
        ...
        col_layers_info: Optional[List[tuple]] = None,
        row_layers_info: Optional[List[tuple]] = None,
        predictor_layers_info: Optional[List[tuple]] = None,
        ...
        self.col_layers_info = col_layers_info
        self.row_layers_info = row_layers_info
        self.predictor_layers_info = predictor_layers_info
        ...
        for X_batch, y_batch, pattern_batch in zip(Xs, ys, shuffle_patterns):
            ...
            with torch.no_grad():
                out = self.model_(
                    ...
                    col_layers_info=self.col_layers_info,
                    row_layers_info=self.row_layers_info,
                    predictor_layers_info=self.predictor_layers_info,
                )
    ...
    def predict_with_proba(self, X):
        proba = self.predict_proba(X)
        y = np.argmax(proba, axis=1)
        return self.y_encoder_.inverse_transform(y), proba
    ...
    def get_feature_encoder_embeddings(self, raw_embeddings = False):
        try:
            device = next(self.model_.parameters()).device
            features = self.model_.col_embedder.tf_col.blocks[self.col_layers_info[0][0]].in_embeddings.to(device)
            l_embd = features.float()  # (B, T, E)
            weights = self.model_.col_embedder.ln_w(self.model_.col_embedder.out_w(l_embd))  # (..., T, E)
            biases = self.model_.col_embedder.ln_b(self.model_.col_embedder.out_b(l_embd))  # (..., T, E)
            embedding_ = (features * weights + biases).transpose(0, 1).unsqueeze(0)
            cls_outputs = embedding_[..., : self.model_.row_interactor.num_cls, :].clone()  # (B, T, C, E)
            cls_outputs = self.model_.row_interactor.out_ln(cls_outputs.to(device))
            embeddings = cls_outputs.flatten(-2) # (B, T, C*E)
        except Exception as e:
            if "out of memory" in str(e):
                print("CUDA out of memory! Clearing cache and switching to CPU...")
                torch.cuda.empty_cache()
                embeddings = "error: CUDA out of memory"
            else:
    ...
    def get_decoder(self):
        self.model_init()
        return self.model_.icl_predictor.decoder

    def get_row_layers_embeddings(self, raw_embeddings = False) -> List[np.ndarray]:
        embeddings = []
        for l, info in self.row_layers_info:
            layer = self.model_.row_interactor.tf_row.blocks[l]
            if raw_embeddings:
                embeddings.append(layer.out_embeddings)
            else:
                device = next(self.model_.parameters()).device      
                cls_outputs = layer.out_embeddings[..., : self.model_.row_interactor.num_cls, :].clone()  # (B, T, C, E)
                cls_outputs = self.model_.row_interactor.out_ln(cls_outputs.to(device))
                embeddings.append(cls_outputs.flatten(-2))  # (B, T, C*E)
        return embeddings

    def get_col_layers_embeddings(self, raw_embeddings = False) -> List[np.ndarray]:
        embeddings = []
        for l, info in self.col_layers_info:
            layer = self.model_.col_embedder.tf_col.blocks[l]
            if raw_embeddings:
                embeddings.append(layer.out_embeddings)
            else:
                try:
                    device = next(self.model_.parameters()).device      
                    features = self.model_.col_embedder.tf_col.blocks[0].in_embeddings.to(device)
                    l_embd = layer.out_embeddings.to(device).float()  # (B, T, E)
                    weights = self.model_.col_embedder.ln_w(self.model_.col_embedder.out_w(l_embd))  # (..., T, E)
                    biases = self.model_.col_embedder.ln_b(self.model_.col_embedder.out_b(l_embd))  # (..., T, E)
                    embedding_ = (features * weights + biases).transpose(0, 1).unsqueeze(0)
                    cls_outputs = embedding_[..., : self.model_.row_interactor.num_cls, :].clone()  # (B, T, C, E)
                    ...
                    if "out of memory" in str(e):
                        print("CUDA out of memory! Clearing cache and switching to CPU...")
                        torch.cuda.empty_cache()
                        embeddings.append("error: CUDA out of memory")
                    else:
                        embeddings.append("error: " + str(e))
        return embeddings

    def get_all_layers_embeddings(self, layers_type = "predictor") -> List[np.ndarray]:
        if layers_type == "row":
            return self.get_row_layers_embeddings()
        elif layers_type == "col":
            return self.get_col_layers_embeddings()
        else:
            embeddings = []
            for l, info in self.predictor_layers_info:
                layer = self.model_.icl_predictor.tf_icl.blocks[l]
                embeddings.append(layer.out_embeddings)
            return embeddings
    ...
    def get_all_contribution_scores(self):
        ...
        for l, info in self.col_layers_info:
            layer = self.model_.col_embedder.tf_col.blocks[l]
            contribution_scores[f'layer_{l}_col'] = layer.component_contribution_scores
        for l, info in self.row_layers_info:
            layer = self.model_.row_interactor.tf_row.blocks[l]
            contribution_scores[f'layer_{l}_row'] = layer.component_contribution_scores
        for l, info in self.predictor_layers_info:
            layer = self.model_.icl_predictor.tf_icl.blocks[l]
            contribution_scores[f'layer_{l}_predictor'] = layer.component_contribution_scores
    ...
    def get_decoder_pre_norm(self):
        return self.model_.icl_predictor.ln

    def get_all_layers_predictions(self, embeddings = None, layers_type = "predictor", norm_first= True, decoder_type = "default") -> (List[np.ndarray], List[np.ndarray]):
        if embeddings is None:
            embeddings = self.get_all_layers_embeddings(layers_type = layers_type)
        class_shift_offsets = []
        for offsets in self.ensemble_generator_.class_shift_offsets_.values():
            class_shift_offsets.extend(offsets)
        n_estimators = len(class_shift_offsets)
        ...
        for l, emb in enumerate(embeddings):
            if isinstance(emb, str) and "error" in emb:
                all_layer_prediction.append(emb)
                all_layer_predict_proba.append(emb)
                continue
            if emb.shape[-1] != 512:
                all_layer_prediction.append(
                    f"error: embedding dimension {emb.shape[1]} is not compatible with the decoder input dimension 512"
                )
                all_layer_predict_proba.append(
                    f"error: embedding dimension {emb.shape[1]} is not compatible with the decoder input dimension 512"
                )
                continue
            ...
            device = next(self.model_.parameters()).device
            if norm_first:
                emb = self.model_.icl_predictor.ln(emb.to(device).float())
            
            if decoder_type ==  "default":
                decoder = self.get_decoder()
            if decoder_type ==  "finetuned" and layers_type == "predictor":
                decoder = self.finetuned_decoders[l].to(device)

            logits = decoder(emb.to(device).float()).detach().cpu().numpy()

            outputs = logits[:, :, :self.model_.icl_predictor.num_classes]
            avg = None
            for i, offset in enumerate(class_shift_offsets):
                out = outputs[i]
                out = np.concatenate([out[..., offset:], out[..., :offset]], axis=-1)
            ...
            if self.average_logits:
                avg = self.softmax(avg, axis=-1, temperature=self.softmax_temperature)
            p =  avg / avg.sum(axis=1, keepdims=True)

            all_layer_predict_proba.append(p)
            y = np.argmax(p, axis=-1)
            all_layer_prediction.append(self.y_encoder_.inverse_transform(y))
    ...
    def model_inference(self,  X, y , support_size, training_mode = True):
    
        y_full =y.float().to(self.device_)
        X_full = X.float().to(self.device_)
        if training_mode:
            self.model_.training = True
            self.model_.icl_predictor.training = True
        output = self.model_(X_full, y_full,
                    col_layers_info=self.col_layers_info,
                    row_layers_info=self.row_layers_info,
                    predictor_layers_info=self.predictor_layers_info)
        if training_mode:
            self.model_.training = False
            self.model_.icl_predictor.training = False
