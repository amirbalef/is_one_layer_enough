...
from tabpfn_v2.base import (
    check_cpu_warning,
    create_inference_engine,
    determine_precision,
    initialize_tabpfn_model,
)
from tabpfn_v2.config import ModelInterfaceConfig
from tabpfn_v2.constants import (
    PROBABILITY_EPSILON_ROUND_ZERO,
    SKLEARN_16_DECIMAL_PRECISION,
    XType,
    YType,
)
from tabpfn_v2.preprocessing import (
    ClassifierEnsembleConfig,
    EnsembleConfig,
    PreprocessorConfig,
    default_classifier_preprocessor_configs,
)
from tabpfn_v2.utils import (
    _fix_dtypes,
    _get_embeddings,
...
if TYPE_CHECKING:
    ...
    from tabpfn_v2.inference import InferenceEngine
    from tabpfn_v2.model.config import InferenceConfig
...
class TabPFNClassifier(ClassifierMixin, BaseEstimator):
    ...
    def load_checkpoint(self, model_weights: str, device ) -> None:
    ...
    def fit(self, X: XType, y: YType) -> Self:
        ...
        if self.model_weights is not None:
            self.load_checkpoint(self.model_weights, device = self.device_)
    ...
    def predict_with_proba(self, X: XType) -> np.ndarray:
        proba = self.predict_proba(X)
        y = np.argmax(proba, axis=1)
        return self.label_encoder_.inverse_transform(y), proba  # type: ignore
    ...
    def model_inference(self,  X, y , support_size):
        ...
        y_full = torch.cat([y, torch.zeros( (X.shape[0], X.shape[1] - support_size) , device=y.device, dtype=y.dtype)],dim=1).transpose(0, 1).contiguous().to(self.device_)
        X_full = X.transpose(0, 1).contiguous().to(self.device_)
        output = self.model_(X_full, y_full,single_eval_pos=int(support_size), layers_info = self.layers_info)
    ...
    def get_feature_encoder_embeddings(self): #todo fix for n_estimators>1
        return [self.model_.transformer_encoder.layers[self.layers_info[0][0]].in_embeddings]
    ...
    def info() -> dict[str, Any]:
        info = {}
        info["model_name"] = "TabPFN (v2)"
        info["n_layers"] = 12
        info["attention_heads"] = 8
        info["hidden_size"] = 256
        info["max_features"] = 500
        info["max_classes"] = 10
        info["pretraining_dataset"] = "Synthetic datasets generated from a large mixture of generative processes."
