...
class LoopedNanoTabPFN(nn.Module):
    ...
    def _forward(self, src: Tuple[torch.Tensor, torch.Tensor], single_eval_pos: int, num_mem_chunks: int = 1, layers_info = None) -> torch.Tensor:
        x_src, y_src = src
        ...
        output = self.transformer_encoder(src, single_eval_pos, num_mem_chunks=num_mem_chunks, layers_info = layers_info)
...
class TransformerEncoderStack(nn.Module):
    def __init__(self, num_layers: int, embedding_size: int, num_attention_heads: int, mlp_hidden_size: int):
        ...
        super().__init__()
        self.transformer_block = TransformerEncoderLayer(embedding_size, num_attention_heads, mlp_hidden_size)
        self.num_layers = num_layers

    def forward(self, x: torch.Tensor, single_eval_position: int, num_mem_chunks: int = 1, layers_info = None) -> torch.Tensor:
        """
        ...
        for _ in range(self.num_layers):
            self.transformer_blocks.append(copy.deepcopy(self.transformer_block))

        if layers_info is not None:
            for l, task_info in layers_info:
                block = self.transformer_blocks[l]
                x = block(x, single_eval_position=single_eval_position, num_mem_chunks=num_mem_chunks, task_info=task_info)
        else:
...
class TransformerEncoderLayer(nn.Module):
    ...
    def compute_contribution(out_np, src_np, single_eval_position: int):
        ...
        contribution_scores["norm"]["total"] = (np.linalg.norm(out_np) / np.linalg.norm(src_np))
        contribution_scores["norm"]["support"] = (np.linalg.norm(out_np[:single_eval_position])/ np.linalg.norm(src_np[:single_eval_position]))
        contribution_scores["norm"]["query"] = (np.linalg.norm(out_np[single_eval_position:])/ np.linalg.norm(src_np[single_eval_position:]))
    ...
    def forward(self, src: torch.Tensor, single_eval_position: int, num_mem_chunks: int = 1, task_info={}) -> torch.Tensor:
        """
        ...
        def feature_attention(x):
            return   self.self_attention_between_features(x, x, x)[0]

        out = src.reshape(batch_size*rows_size, col_size, embedding_size).clone()
        out = feature_attention(out)
        out = out.reshape(batch_size, rows_size, col_size, embedding_size)

        self.component_contribution("attn_features", out, src, single_eval_position, task_info)
        out[:, :single_eval_position] = out[:, :single_eval_position] * task_info.get("w_"+ "attn_features" + "_on_support", 1.0)
        out[:, single_eval_position:] = out[:, single_eval_position:] * task_info.get("w_"+ "attn_features" + "_on_query", 1.0)
        ...
        out = src.transpose(1, 2).clone()
        out = out.reshape(batch_size*col_size, rows_size, embedding_size)
        out = datapoint_attention(out)
        out = out.reshape(batch_size, col_size, rows_size, embedding_size)
        out = out.transpose(2, 1)
        self.component_contribution("attn_items", out, src, single_eval_position, task_info)
        out[:, :single_eval_position] = out[:, :single_eval_position] * task_info.get("w_"+ "attn_items" + "_on_support", 1.0)
        out[:, single_eval_position:] = out[:, single_eval_position:] * task_info.get("w_"+ "attn_items" + "_on_query", 1.0)
        src = out + src
        ...
        out = mlp(src.reshape(-1, embedding_size).clone())
        out = out.reshape(batch_size, rows_size, col_size, embedding_size)
        self.component_contribution("mlp", out, src, single_eval_position, task_info)
        out[:, :single_eval_position] = out[:, :single_eval_position] * task_info.get("w_"+ "mlp" + "_on_support", 1.0)
        out[:, single_eval_position:] = out[:, single_eval_position:] * task_info.get("w_"+ "mlp" + "_on_query", 1.0)
        src = out + src

        src = self.norm3(src)
        self.out_embeddings = src.clone().detach().cpu()
        in_embeddings = self.in_embeddings.clone().to(device=src.device)
        in_embeddings =  self.norm3(self.norm2(self.norm1(in_embeddings))).detach().cpu()
        self.component_contribution("layer", (self.out_embeddings - in_embeddings), (in_embeddings), single_eval_position, task_info)    
