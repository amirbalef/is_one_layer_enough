...
class OneLayerNanoTabPFN(nn.Module):
...
class TransformerEncoderStack(nn.Module):
    def __init__(self, num_layers: int, embedding_size: int, num_attention_heads: int, mlp_hidden_size: int):
        ...
        self.transformer_blocks = [self.transformer_block]
    ...
    def forward(self, x: torch.Tensor, single_eval_position: int, num_mem_chunks: int = 1, layers_info = None) -> torch.Tensor:
        ...
        if layers_info is not None:
            for l, task_info in layers_info:
                x = self.transformer_block(x, single_eval_position=single_eval_position, num_mem_chunks=num_mem_chunks, task_info=task_info)
        else:
            for block in self.transformer_blocks:
                x = self.transformer_block(x, single_eval_position=single_eval_position, num_mem_chunks=num_mem_chunks)
        return x
