...
from tabpfn_v2_5.architectures.base.encoders import (
    LinearInputEncoderStep,
    NanHandlingEncoderStep,
    SequentialEncoder,
)
from tabpfn_v2_5.architectures.base.layer import PerFeatureEncoderLayer
from tabpfn_v2_5.architectures.base.thinking_tokens import AddThinkingTokens
from tabpfn_v2_5.architectures.interface import Architecture

if TYPE_CHECKING:
    from tabpfn_v2_5.architectures.base.config import ModelConfig
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if layers_info is None:
            n_layers = torch.randint(low=self.min_num_layers_layer_dropout, high=len(self.layers) + 1, size=(1,)).item()
            ...
            for l, info  in layers_info:
                layer = self.layers[l]
                if recompute_layer and x.requires_grad:
                    x = checkpoint(partial(layer, task_info = info, **kwargs), x, use_reentrant=False)
                else:
                    x = layer(x, task_info = info, **kwargs)  
            return x
