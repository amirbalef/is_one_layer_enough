...
from tabpfn_v2_5.constants import (
...
from tabpfn_v2_5.inference import InferenceEngine, InferenceEngineBatchedNoPreprocessing
from tabpfn_v2_5.inference_tuning import (
...
from tabpfn_v2_5.preprocessing import (
    ClassifierEnsembleConfig,
    DatasetCollectionWithPreprocessing,
    EnsembleConfig,
    PreprocessorConfig,
)
from tabpfn_v2_5.preprocessors.preprocessing_helpers import get_ordinal_encoder
from tabpfn_v2_5.utils import (
...
if TYPE_CHECKING:
    ...
    from tabpfn_v2_5.architectures.base.memory import MemorySavingMode
    from tabpfn_v2_5.architectures.interface import Architecture, ArchitectureConfig
    from tabpfn_v2_5.inference_config import InferenceConfig
...
class TabPFNClassifier(ClassifierMixin, BaseEstimator):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        layers_info: list[tuple[int, dict[str, float]]] | None = None,
    ...
    def predict_with_proba(self, X: XType) -> tuple[np.ndarray, np.ndarray]:
        probas = self._predict_proba(X=X)
        y_pred = np.argmax(probas, axis=1)
        if hasattr(self, "label_encoder_") and self.label_encoder_ is not None:
            return self.label_encoder_.inverse_transform(y_pred), probas
    ...
    def get_all_contribution_scores(self):
        ...
        for l, info in self.layers_info:
            layer = self.model_.transformer_encoder.layers[l]
    ...
    def get_all_layers_embeddings(self): #todo fix for n_estimators>1
        ...
        for l, info in self.layers_info:
            layer = self.model_.transformer_encoder.layers[l]
            embeddings.append(layer.out_embeddings[:,self.model_.num_thinking_rows:,:])
        return embeddings
    ...
    def model_init(self):
        if not hasattr(self, "models_"):
            byte_size, rng = self._initialize_model_variables()
            self.models_[0] = self.models_[0].to(self.devices_[0])

    def model_inference(self,  X, y , support_size):
    
        y_full =y.transpose(0, 1).contiguous().to(self.devices_[0])
        X_full = X.transpose(0, 1).contiguous().to(self.devices_[0])
        output = self.model_(X_full, y_full, layers_info = self.layers_info)
        
        return output

    def get_decoder(self):
        self.model_init()
        return self.model_.decoder_dict["standard"]
    ...
    def get_all_layers_predictions(self, embeddings=None, decoder_type = "default"): 
        ...
        for l, emb in enumerate(embeddings):
            ...
            for output,  config in zip(decoder_outputs, self.executor_.ensemble_configs):
            ...
            output = self.logits_to_probabilities(outputs)

            output = self._maybe_reweight_probas(probas=output)
            ...
            p =  output / output.sum(axis=1, keepdims=True)  # type: ignore
    ...
    def get_feature_encoder_embeddings(self): #todo fix for n_estimators>1
        return [self.model_.transformer_encoder.layers[self.layers_info[0][0]].in_embeddings[:,self.model_.num_thinking_rows:,:]]
